...
serve(async (req) => {
  ...
  try {
    ...
    // Create a function to directly execute SQL since we need service role admin permissions
    const { data: execFnData, error: execFnError } = await supabaseAdmin.rpc('exec_sql', {
      sql: `
    ...
    if (execFnError) {
      ...
      const { error } = await supabaseAdmin.from('_temp_exec').select().limit(1);
      if (error) {
        console.log("Error setting up exec_sql function:", error);
      }
    }

    // Now that we have the exec_sql function, we can use it to execute our table and function creation SQL
